  ...
  const validateSSOKey = useCallback(async (ssoKey: string, email: string): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage =
        ...
        'Failed to validate SSO credentials';
  ...
  const performSSOLogin = useCallback(
    async (
      ...
      if (!ssoKey || !gmail || !(validationResult?.success || validationResult?.valid)) {
        ...
        setLoginState({
          step: 'error',
          message: 'SSO credentials validation required',
          progress: 0,
        });
      ...
      try {
        ...
        if (result.data && result.data.user) {
          ...
          try {
            ...
            setLoginState({
              step: 'success',
              message: 'Login successful! (Using fallback token) Redirecting...',
              progress: 100,
            });
        ...
        const errorMessage =
          err?.response?.data?.error || err?.response?.data?.message || 'SSO login failed';
        setError(errorMessage);
        setLoginState({ step: 'error', message: errorMessage, progress: 0 });
