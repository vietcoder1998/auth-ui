...
const AdminFaqCreateModal: React.FC<AdminFaqCreateModalProps> = ({ open, onCancel, onOk, loading, promptOptions }) => {
    ...
    >
      <Form form={form} layout="vertical" initialValues={{ type: 'manual' }}>
        <Form.Item name="question" label="Question" rules={[{ required: true, message: 'Please enter the question' }]}> 
          <AIGenerateInput textarea rows={2} placeholder="Enter question or generate with AI..." prompt="Generate a FAQ question" />
        </Form.Item>
        <Form.Item name="answer" label="Answer" rules={[{ required: true, message: 'Please enter the answer' }]}> 
          <AIGenerateInput textarea rows={3} placeholder="Enter answer or generate with AI..." prompt="Generate a FAQ answer" />
        </Form.Item>
        ...
        {promptOptions && (
          <Form.Item name="promptId" label="Prompt History" rules={[{ required: false }]}> 
            <Select options={promptOptions} showSearch allowClear placeholder="Select prompt history (optional)" />
          </Form.Item>
        )}
      </Form>
    </Modal>
